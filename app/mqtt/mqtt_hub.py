...
class MqttHub:
    ...
    def __init__(self, app: Flask, config_path):
        ...
        app.config['MQTT_USERNAME'] = self.config["mqtt_username"]      # set the username here if you need authentication for the broker
        app.config['MQTT_PASSWORD'] = self.config["mqtt_pass"]          # set the password here if the broker demands authentication
        app.config['MQTT_KEEPALIVE'] = self.config["mqtt_keepalive"]    # set the time interval for sending a ping to the broker to 5 seconds
